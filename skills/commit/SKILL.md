...
name: commit
description: |
  Prepare a Conventional Commit scoped with Jira ID before running git commit.
...
4. **Context Extraction**:
   - Attempt to extract the Jira ID from the current branch name.
...
5. **Message Preparation**:
   ...
   - Ensure `scope` includes the component and Jira ID if present (e.g., `api, PROJ-123`).
   - If no Jira ID is found in the branch (non-Jira repos), use just the component scope.
